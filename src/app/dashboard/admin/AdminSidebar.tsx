...
export default function AdminSidebar({
  ...
  return (
    <div className="min-h-screen bg-[#fcfcfc] font-sans selection:bg-blue-100 selection:text-blue-900">
      {/* MOBILE HEADER */}
              ...
              <nav className="flex-1 overflow-y-auto p-6 space-y-2 no-scrollbar">
                {menuItems.map((item) => (
                  <Link
                    ...
                    href={item.href}
                    onClick={() => setSidebarOpen(false)}
                    className={`flex items-center justify-between px-4 py-4 rounded-2xl transition-all ${item.isActive ? "bg-blue-700 text-white shadow-xl shadow-blue-200 font-bold" : "text-slate-500 hover:bg-stone-50 hover:text-blue-900"}`}
                  >
                    <div className="flex items-center gap-4">
                      <item.icon
      ...
      >
            ...
            <div className="px-2 mb-2">
              <div className="relative group">
                ...
                <select
                  ...
                  className="w-full appearance-none bg-stone-50 border border-stone-100 text-[10px] font-black text-blue-900 uppercase tracking-widest pl-10 pr-8 py-3 rounded-xl cursor-pointer focus:outline-none focus:ring-4 focus:ring-blue-500/5 transition-all group-hover:bg-stone-100"
                >
                ...
                <ChevronRight className="absolute right-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-blue-400 pointer-events-none group-hover:text-blue-900 transition-colors" />
              </div>
            </div>
          )}
            ...
            {menuItems.map((item, idx) => {
              ...
              return (
                <div key={item.name} className="relative">
                  ...
                  <Link
                    href={item.href}
                    className={`relative group flex items-center justify-between px-4 py-3.5 rounded-2xl transition-all duration-300 ${item.isActive ? "text-blue-950 font-black" : "text-slate-500 hover:text-blue-900 hover:bg-stone-50"}`}
                  >
                    {item.isActive && (
                      <motion.div
                    ...
                    <div className="flex items-center gap-4">
                      <item.icon
                        className={`w-5 h-5 shrink-0 transition-all duration-300 ${item.isActive ? "text-blue-700 scale-110" : "text-slate-400 group-hover:text-blue-600"}`}
                      />
                      {!collapsed && (
        ...
        <div className="p-6 border-t border-slate-50 bg-stone-50/30">
          <button
            ...
            className={`w-full flex items-center justify-center p-3 rounded-xl text-slate-400 hover:text-blue-950 hover:bg-white hover:shadow-sm mb-4 transition-all duration-300 active:scale-95 ${collapsed ? "" : "gap-3"}`}
          >
            <PanelLeft
        ...
        <header className="hidden lg:flex sticky top-0 z-40 bg-white/80 backdrop-blur-2xl border-b border-slate-100 h-24 items-center justify-between px-12">
          <div className="flex items-center gap-10">
            <div className="flex items-center gap-3 text-[11px] font-black text-stone-400 uppercase tracking-widest">
              ...
              <span className="text-blue-950 italic">Overview</span>
            </div>
            <div className="h-6 w-px bg-stone-100" />
            <div className="relative group">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-stone-300 group-focus-within:text-blue-600 transition-colors" />
              <input
                type="text"
                placeholder="Search system data..."
                className="bg-stone-50 border-stone-100 rounded-[1.25rem] pl-11 pr-6 py-3 text-[13px] w-80 focus:w-[450px] focus:bg-white focus:ring-4 focus:ring-blue-500/5 focus:border-blue-100 transition-all duration-500 placeholder:text-stone-300 font-medium"
              />
            </div>
          </div>
          <div className="flex items-center gap-6">
            <LanguageSwitcher />
            <div className="h-6 w-px bg-stone-100" />
            <button className="relative p-3 text-stone-400 hover:text-blue-700 hover:bg-blue-50 rounded-2xl transition-all active:scale-95">
