...
export async function GET() {
    try {
        ...
        for (const jadwal of jadwals) {
            ...
            } else if ((session.title === "Wawancara Calsan" || session.title === "Wawancara Santri") && !jadwal.penguji_santri_id) {
                updateData.penguji_santri_id = session.created_by;
            } else if ((session.title === "Wawancara Cawalsan" || session.title === "Wawancara Orangtua/Wali") && !jadwal.penguji_ortu_id) {
