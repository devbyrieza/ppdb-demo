...
export async function POST(request: Request) {
    ...
    try {
        ...
        if (!isAssigned && !isCreator) {
            return NextResponse.json({ error: "You do not have permission to cancel this schedule" }, { status: 403 });
        }
        ...
        if (jadwal.exam_session?.title) {
            ...
            jenisUjian = "Wawancara Calsan";
        } else if (jadwal.penguji_quran_id === userId) {
            jenisUjian = "Tes Al-Qur'an";
        } else if (jadwal.penguji_ortu_id === userId) {
            jenisUjian = "Wawancara Cawalsan";
        }

        const dateFormatted = new Date(jadwal.tanggal_ujian).toLocaleDateString('id-ID', {
            weekday: 'long',
            day: 'numeric',
            month: 'long',
            year: 'numeric'
        ...
        const timeFormatted = new Date(jadwal.exam_session?.start_time || jadwal.waktu_mulai_santri).toLocaleTimeString('id-ID', {
            hour: '2-digit',
            minute: '2-digit'
        });
