...
function testBulkAssignLogic(currentStatus: string) {
    const updatedStatus = hasReachedStatus(currentStatus as StatusProses, 'scheduled') 
        ? currentStatus 
        : 'scheduled';
...
cases.forEach(c => {
    ...
    console.log(`[${pass ? 'PASS' : 'FAIL'}] Input: ${c.input.padEnd(15)} -> Result: ${result.padEnd(12)} (Expected: ${c.expected})`);
