...
const variantConfig = {
    success: {
        ...
        containerFilled: "bg-[var(--color-success-500)] border-transparent text-white",
        ...
        dismissColor: "text-[var(--color-success-500)]/50 hover:text-[var(--color-success-500)] hover:bg-[var(--color-success-100)]",
    ...
    error: {
        ...
        containerFilled: "bg-[var(--color-danger-500)] border-transparent text-white",
        ...
        bodyColorFilled: "text-white/90",
        dismissColor: "text-[var(--color-danger-500)]/50 hover:text-[var(--color-danger-500)] hover:bg-[var(--color-danger-100)]",
        dismissColorFilled: "text-white/60 hover:text-white hover:bg-white/20",
    },
    warning: {
        container: "bg-[var(--color-warning-50)] border-[var(--color-warning-100)]",
        containerFilled: "bg-[var(--color-warning-500)] border-transparent text-white",
        ...
        bodyColorFilled: "text-white/90",
        dismissColor: "text-[var(--color-warning-500)]/50 hover:text-[var(--color-warning-500)] hover:bg-[var(--color-warning-100)]",
        dismissColorFilled: "text-white/60 hover:text-white hover:bg-white/20",
    },
    // info → army-tinted, konsisten dengan branding hijau army template-demo
    info: {
        container: "bg-[var(--color-army-50)] border-[var(--color-army-100)]",
        containerFilled: "bg-[var(--color-army-700)] border-transparent text-white",
        accent: "border-l-[var(--color-army-600)]",
        iconWrapper: "bg-[var(--color-army-100)]",
        iconWrapperFilled: "bg-white/20",
        icon: Info,
        iconColor: "text-[var(--color-army-700)]",
        iconColorFilled: "text-white",
        titleColor: "text-[var(--color-army-800)]",
        titleColorFilled: "text-white",
        bodyColor: "text-[var(--color-army-700)]/80",
        bodyColorFilled: "text-white/90",
        dismissColor: "text-[var(--color-army-600)]/50 hover:text-[var(--color-army-700)] hover:bg-[var(--color-army-100)]",
        dismissColorFilled: "text-white/60 hover:text-white hover:bg-white/20",
    },
} as const;
            ...
            {visible && (
                <motion.div
                    ...
                    className={cn(
                        ...
                        sz.container,
                        accent && !filled
                            ? [sz.accentBorder, cfg.accent, "border-t-0 border-r-0 border-b-0 rounded-l-none"]
                            : "",
                ...
                >
                    <div className={cn(
                        ...
                        <IconComponent
                            className={cn(sz.icon, filled ? cfg.iconColorFilled : cfg.iconColor)}
                            strokeWidth={2}
                        />
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                        {title && (
                            <p className={cn(sz.title, "mb-0.5", filled ? cfg.titleColorFilled : cfg.titleColor)}>
                                {title}
                            </p>
                        )}
                        <div className={cn(
...
export function AlertSuccess(props: Omit<AlertProps, "type">) { return <Alert {...props} type="success" />; }
export function AlertError(props: Omit<AlertProps, "type">) { return <Alert {...props} type="error" />; }
export function AlertWarning(props: Omit<AlertProps, "type">) { return <Alert {...props} type="warning" />; }
export function AlertInfo(props: Omit<AlertProps, "type">) { return <Alert {...props} type="info" />; }

interface InlineAlertProps { type?: AlertVariant; children: React.ReactNode; className?: string; }
...
export function InlineAlert({ type = "error", children, className }: InlineAlertProps) {
    ...
    return (
        <motion.p
            ...
            className={cn("flex items-center gap-1.5 text-xs font-medium", cfg.iconColor, className)}
        >
...
interface AlertBannerProps { type?: AlertVariant; children: React.ReactNode; dismissible?: boolean; className?: string; }

export function AlertBanner({ type = "warning", children, dismissible = true, className }: AlertBannerProps) {
    const [visible, setVisible] = useState(true);
    const cfg = variantConfig[type];
    const IconComponent = cfg.icon;
    return (
            ...
            {visible && (
                ...
                >
                    <div className={cn(
                        ...
                        cfg.containerFilled, className
                    )}>
                        <div className="flex items-center gap-2 flex-1 min-w-0">
                            <IconComponent className="w-4 h-4 shrink-0 text-white" strokeWidth={2} />
