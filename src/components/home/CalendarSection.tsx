        ...
        <div className="flex flex-col lg:flex-row gap-16 lg:gap-24">
          ...
          <div className="lg:w-[55%]">
            ...
            >
              ...
              <h2 className="section-title mb-10">
                Timeline <span className="text-gradient-army">Terstruktur</span>
              </h2>

              <div className="space-y-8 relative">
                {/* Vertical Line */}
                <div className="absolute left-[23px] top-6 bottom-6 w-0.5 bg-khaki-200" />
                ...
                {IMPORTANT_DATES.map((item, idx) => (
                  ...
                  >
                    <div className={`w-12 h-12 rounded-[14px] flex items-center justify-center relative z-10 shrink-0 shadow-sm transition-transform duration-500 group-hover:scale-110 ${
                        item.color === 'army' ? 'bg-army-700 text-white' :
                        item.color === 'khaki' ? 'bg-khaki-200 text-army-800 border border-khaki-300' :
                        'bg-khaki-500 text-white border border-khaki-600'
                      }`}>
                    ...
                    <div className="app-card bg-surface-50 p-6 md:p-8 flex-grow hover:bg-white transition-all duration-300">
                      <div className="flex items-center justify-between gap-4 mb-4">
                        <span className={`px-3 py-1.5 rounded-pill text-[10px] font-bold uppercase tracking-widest ${
                            item.color === 'army' ? 'bg-army-50 text-army-700 border border-army-100' :
                            item.color === 'khaki' ? 'bg-khaki-100 text-army-800 border border-khaki-200' :
                            'bg-khaki-50 text-army-700 border border-khaki-200'
                          }`}>
                          {item.date}
                        </span>
                      </div>
                      <h4 className="font-bold text-ink-950 text-xl mb-2 tracking-tight group-hover:text-army-700 transition-colors">{item.title}</h4>
          ...
          <div className="lg:w-[45%]">
            <motion.div
              ...
              whileInView={{ opacity: 1, x: 0 }}
              viewport={{ once: true }}
              className="bg-white rounded-[2.5rem] p-8 sm:p-10 md:p-14 flex flex-col justify-center items-center text-center relative overflow-hidden shadow-lg border border-khaki-200 h-full hover:shadow-xl transition-all duration-500"
            >
              <div className="w-20 h-20 sm:w-24 sm:h-24 bg-khaki-50 rounded-full flex items-center justify-center mb-8 shadow-sm">
                <Clock className="w-10 h-10 sm:w-12 sm:h-12 text-army-600" />
              </div>
              ...
              {/* Decorative blobs */}
              <div className="absolute -bottom-24 -right-24 w-72 h-72 bg-khaki-200/50 rounded-full blur-[80px] -z-10" />
              <div className="absolute -top-24 -left-24 w-72 h-72 bg-army-50 rounded-full blur-[80px] -z-10" />
            </motion.div>
          </div>
