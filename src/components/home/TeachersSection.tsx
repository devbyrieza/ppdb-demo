        ...
        <section id="pengajar" className="section-alt">
            ...
            <Container className="relative z-10">
                <div className="flex flex-col lg:flex-row gap-16 lg:gap-24 items-start">
                    ...
                    >
                        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-pill bg-white border border-sand-200 text-teal-700 text-xs font-bold uppercase tracking-widest mb-6 shadow-sm">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-5">
                            {[
                                { icon: Globe, label: "Lulusan Luar Negeri", sub: "Timur Tengah, Mesir & Maroko", color: "teal" },
                                { icon: Users, label: "Native Speakers", sub: "Timur Tengah", color: "sand" },
                                { icon: BookOpen, label: "Lulusan Terbaik", sub: "Dalam Negeri & Pondok Unggulan", color: "gold" },
                                { icon: Award, label: "Dosen & Pakar", sub: "Tenaga Pendidik Profesional", color: "teal" }
                            ].map((feature, i) => (
                                <div key={i} className="app-card p-4 flex items-center gap-5">
                                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 shadow-sm transition-transform duration-300 hover:scale-110 ${
                                        ...
                                        feature.color === 'sand' ? 'bg-sand-100 text-teal-800' :
                    ...
                    <div className="lg:w-[60%] grid grid-cols-1 sm:grid-cols-2 gap-4 w-full">
                        {TEACHER_BACKGROUNDS.map((item, idx) => (
                            ...
                            >
                                <div className="mt-1 w-6 h-6 rounded-full bg-sand-100 flex items-center justify-center shrink-0 shadow-sm transition-transform group-hover:scale-110">
                                    <CheckCircle className="w-3.5 h-3.5 text-teal-700" />
                                </div>
                                <div className="space-y-1.5 mt-0.5">
                                    <h4 className="font-bold text-ink-900 text-[15px] leading-snug group-hover:text-teal-800 transition-colors">{item.institution}</h4>
