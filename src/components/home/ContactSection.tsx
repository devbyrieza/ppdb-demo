    ...
    >
      <Container>
        <motion.div
          ...
          className="bg-secondary-50/50 rounded-[2rem] lg:rounded-[2.5rem] p-8 md:p-12 lg:p-14 border border-secondary-200 relative overflow-hidden shadow-sm"
        >
          ...
          <div className="relative z-10 flex flex-col lg:flex-row gap-12 lg:gap-16 items-center">
            ...
            <div className="lg:w-[55%] grid sm:grid-cols-2 gap-3 sm:gap-4 w-full">
              {CONTACT_INFO.map((item, idx) => (
                <div
                  key={idx}
                  className={`bg-white p-6 rounded-2xl border border-secondary-200 shadow-premium-sm hover:shadow-premium-md flex flex-col items-start group transition-all duration-400 ${idx === 0 ? "sm:col-span-2" : ""}`}
                >
                  <div
