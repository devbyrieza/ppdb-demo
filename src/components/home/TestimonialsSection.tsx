...
function TestimonialCard({
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -5 }}
      className="bg-white p-8 rounded-[2.5rem] h-full flex flex-col relative group transition-all duration-500 border border-khaki-200/60 hover:border-army-200 hover:shadow-premium-xl overflow-hidden"
    >
      {/* Decorative Gradient Background on Hover */}
      <div className="absolute inset-0 bg-linear-to-br from-transparent to-army-50/30 opacity-0 group-hover:opacity-100 transition-opacity duration-500" />

      {/* Number badge */}
      <div className="absolute top-8 left-8 w-10 h-10 rounded-[14px] bg-khaki-50 border border-khaki-200 flex items-center justify-center shadow-premium-xs group-hover:scale-110 transition-transform duration-500 z-10">
        <span className="text-[10px] font-black text-army-700 tracking-wider">#{no}</span>
      </div>

      <Quote className="absolute top-8 right-8 w-12 h-12 text-khaki-50 group-hover:text-khaki-100 transition-colors duration-500 -rotate-12" />

      {/* Stars with premium glow */}
      <div className="flex gap-1.5 mb-6 mt-14 relative z-10">
        {[...Array(5)].map((_, i) => (
          <Star key={i} className="w-4 h-4 text-khaki-500 fill-khaki-500 drop-shadow-[0_0_8px_rgba(234,179,8,0.3)]" />
        ))}
      </div>
      ...
      <div className="flex flex-col gap-5 mt-auto pt-6 border-t border-khaki-100 relative z-10">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-2xl bg-linear-to-br from-army-700 to-army-900 flex items-center justify-center text-white font-display font-black shadow-premium-sm group-hover:shadow-premium-md group-hover:rotate-6 transition-all duration-500">
            {initial}
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-[15px] font-bold text-ink-950 leading-tight mb-1 truncate">{name}</p>
            <p className="text-[10px] text-army-800 font-bold uppercase tracking-widest truncate">{role}</p>
          </div>
        </div>
        <div className="flex items-center justify-between">
          <p className="text-[10px] text-ink-500 font-semibold tracking-wide">{city}</p>
          <span className="text-[9px] text-army-700 font-black uppercase tracking-widest bg-army-50 px-2.5 py-1 rounded-full border border-army-100/50 shadow-premium-xs">{date}</span>
        </div>
      </div>
    </motion.div>
  );
...
export default function TestimonialsSection() {
  return (
    <section id="testimonials" className="section-alt border-y border-khaki-200">
      <div className="absolute inset-0 bg-[url('/grid-pattern.svg')] opacity-[0.02] pointer-events-none" />
      <div className="absolute top-0 left-0 w-[500px] h-[500px] bg-khaki-100/60 rounded-full blur-[100px] -translate-y-1/2 -translate-x-1/2 opacity-50" />
      ...
      <Container className="relative z-10">
        ...
        <div className="text-center mb-16 lg:mb-20 max-w-2xl mx-auto">
          ...
          >
            Cerita Keberhasilan <span className="text-gradient-army">Keluarga PPDB Modern</span>
          </motion.h2>
          ...
          <motion.div
            ...
            className="inline-flex items-center gap-2 bg-army-50 text-army-800 px-4 py-2 rounded-xl border border-army-100 font-bold text-sm shadow-sm"
          >
            <span className="text-army-600">✦</span> Reputasi Global yang Teruji
          </motion.div>
        </div>
