...
const ACTIVITIES = [
    { name: "Pembelajaran Aktif", color: "teal", description: "Metode interaktif yang memadukan teori dan praktik syar'i guna mengoptimalkan potensi akademik santri secara mendalam.", image: "/images/pembelajaran-kitab-turotz.webp" },
    { name: "Kegiatan Rutin Harian", color: "sand", description: "Pembiasaan ibadah melalui sholat berjamaah tepat waktu and halaqah tahfidz Al-Qur'an setiap hari secara konsisten.", image: "/images/tahfidz.webp" },
    { name: "Ekstrakurikuler Unggulan", color: "teal-alt", description: "Tersedia 15+ pilihan kegiatan mulai dari beladiri hingga Desain Grafis untuk mengasah minat dan bakat santri.", image: "/images/extra-karate.webp" },
    { name: "Kemandirian & Skill", color: "teal-light", description: "Program pelatihan entrepreneurship dan keterampilan hidup mandiri guna mencetak santri yang siap berdikari di masa depan.", image: "/images/luar-kelas.webp" },
] as const;
...
export default function ActivitiesSection() {
    return (
        <section id="kegiatan" className="section-alt border-y border-sand-200/50">
            ...
            <Container className="relative z-10">
                <div className="text-center mb-16 max-w-3xl mx-auto">
                    ...
                    >
                        Kegiatan <span className="text-gradient-teal">Bervariasi & Edukatif</span>
                    </motion.h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
                    {ACTIVITIES.map((activity, idx) => (
                        <motion.div
                            ...
                            className="bg-white rounded-[2rem] border-2 border-transparent hover:border-teal-200 shadow-lg hover:shadow-xl hover:-translate-y-2 transition-all duration-500 group overflow-hidden flex flex-col"
                        >
                            <div className="relative h-48 overflow-hidden shrink-0">
                                <Image
                                    ...
                                    fill
                                    priority={idx < 2}
                                    className="object-cover transition-transform duration-700 group-hover:scale-110 bg-sand-100 animate-pulse"
                                    onLoad={(e) => e.currentTarget.classList.remove('animate-pulse')}
                                />
                                <div className="absolute inset-0 bg-linear-to-t from-black/40 via-transparent to-transparent" />
                            </div>
                            <div className="p-6 md:p-8 grow flex flex-col">
                                <h3 className="text-xl font-bold text-ink-950 mb-3 tracking-tight group-hover:text-teal-700 transition-colors">{activity.name}</h3>
                ...
                <div className="mb-16">
                    ...
                    >
                        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-pill bg-sand-50 border border-sand-200 text-teal-700 text-xs font-bold uppercase tracking-widest shadow-sm">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 md:gap-5">
                        {EXTRA_ACTIVITIES.map((item, idx) => (
                            <motion.div
                                ...
                                className="bg-white p-5 md:p-6 rounded-2xl border border-sand-200 flex flex-col items-center justify-center text-center group hover:bg-sand-50 hover:border-teal-200 hover:shadow-md transition-all duration-500 cursor-default"
                            >
                                <div className={`w-12 h-12 md:w-14 md:h-14 rounded-2xl flex items-center justify-center mb-3 md:mb-4 shadow-sm group-hover:scale-110 transition-transform duration-500 ${
                                    // FIX: Gunakan tipe yang sesuai dengan definisi EXTRA_ACTIVITIES ("teal" | "sand" | "gold")
                                ...
                                <p className="text-[9px] md:text-[10px] font-bold tracking-widest text-ink-950 uppercase group-hover:text-teal-800 transition-colors leading-tight">
                                    {item.name}
                                </p>
                            </motion.div>
                        ))}
